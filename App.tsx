...
import { calculateResultColors, calculateResultSize, generatePeriodId, generateUID, generateMockHistory } from './utils';
...
const INITIAL_GAME_STATE = (mode: GameMode): GameState => ({
    ...
    history: generateMockHistory(mode), // Pre-fill with daily history
    status: 'ACTIVE'
});
...
interface GameContextType extends AppState {
    ...
    gameActions: {
        placeBet: (mode: GameMode, amount: number, choice: BetChoice, type: 'COLOR' | 'NUMBER' | 'SIZE') => void;
    };
    adminActions: {
...
export default function App() {
    ...
    const processGameCompletion = (currentState: AppState, modes: GameMode[]): AppState => {
        ...
        modes.forEach(mode => {
            ...
            const winningSize = calculateResultSize(winningNumber);
            ...
            const updatedBets = newState.bets.map(bet => {
                if (bet.gameMode === mode && bet.periodId === currentPeriodId && bet.status === 'PENDING') {
                    ...
                    // Number Bet
                    if (bet.type === 'NUMBER' && bet.choice === winningNumber) {
                        isWin = true;
                        multiplier = newState.settings.winRatioNumber;
                    }
                    // Color Bet
                    if (bet.type === 'COLOR' && winningColors.includes(bet.choice as string)) {
                        isWin = true;
                        multiplier = newState.settings.winRatioColor;
                    }
                    // Size Bet (Big/Small)
                    if (bet.type === 'SIZE' && bet.choice === winningSize) {
                        isWin = true;
                        multiplier = newState.settings.winRatioSize;
                    }

                    const winAmount = isWin ? Math.floor(bet.amount * multiplier) : 0;
                    
                    return { ...bet, status: isWin ? 'WIN' : 'LOSS', winAmount } as Bet;
                }
                return bet;
            });
    ...
    const gameActions = {
        placeBet: (mode: GameMode, amount: number, choice: BetChoice, type: 'COLOR' | 'NUMBER' | 'SIZE') => {
    ...
    const userActions = {
        ...
        updateProfile: (name: string, password: string, avatar?: string) => {
            if (!state.currentUser) return;
            const updatedUser = { ...state.currentUser, name, password, ...(avatar && { avatar }) };
            setState(prev => ({
                ...prev,
    ...
    const adminActions = {
        ...
        updateAppSettings: (logo: string) => {
            setState(prev => ({
                ...prev,
                settings: { ...prev.settings, appLogo: logo }
            }));
        }
    };
