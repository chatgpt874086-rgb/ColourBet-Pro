...
import { LayoutDashboard, Users, Gamepad2, CreditCard, History, Settings, Check, X, Search, Lock, Unlock, Upload } from 'lucide-react';
...
const AdminGameControl = () => {
    ...
    const betsOnBig = modeBets.filter(b => b.type === 'SIZE' && b.choice === 'big').reduce((a,b) => a + b.amount, 0);
    const betsOnSmall = modeBets.filter(b => b.type === 'SIZE' && b.choice === 'small').reduce((a,b) => a + b.amount, 0);
                         ...
                         <div className="grid grid-cols-2 gap-4 border-b border-slate-700 pb-4 mb-4">
                             <div>
                                 <div className="flex justify-between text-sm mb-1">
                                     <span className="text-orange-400 font-bold">BIG</span>
                                     <span className="text-white">{formatCurrency(betsOnBig)}</span>
                                 </div>
                                 <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                                     <div className="h-full bg-orange-500" style={{width: `${(betsOnBig / (betsOnBig + betsOnSmall + 1)) * 100}%`}}></div>
                                 </div>
                             </div>
                             <div>
                                 <div className="flex justify-between text-sm mb-1">
                                     <span className="text-blue-400 font-bold">SMALL</span>
                                     <span className="text-white">{formatCurrency(betsOnSmall)}</span>
                                 </div>
                                 <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                                     <div className="h-full bg-blue-500" style={{width: `${(betsOnSmall / (betsOnBig + betsOnSmall + 1)) * 100}%`}}></div>
                                 </div>
                             </div>
                         </div>
                         <div>
                        ...
                        {games[mode].history.map(g => (
                            <tr key={g.periodId}>
                                ...
                                <td className="p-3 text-center">
                                    <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded ${g.resultSize === 'big' ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                        {g.resultSize}
                                    </span>
                                </td>
...
const AdminSettings = () => {
    const { settings, adminActions } = useContext(GameContext)!;
    ...
    const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if(e.target.files && e.target.files[0]) {
            ...
            reader.onload = (ev) => {
                if(ev.target?.result) {
                    adminActions.updateAppSettings(ev.target.result as string);
                }
            };
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
                ...
                <div className="flex items-center gap-6">
                    <div className="w-24 h-24 rounded-xl bg-slate-900 border border-slate-700 flex items-center justify-center overflow-hidden">
                        {settings.appLogo ? (
                            <img src={settings.appLogo} className="w-full h-full object-cover" />
                        ) : (
                            <span className="text-xs text-slate-500">No Logo</span>
                        )}
                    </div>
                    <div>
                        <label className="bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg cursor-pointer flex items-center gap-2">
                            <Upload size={16} /> Upload App Logo
                            <input type="file" className="hidden" accept="image/*" onChange={handleLogoUpload} />
                        </label>
                        <p className="text-xs text-slate-500 mt-2">Will be displayed on Login/Auth screens.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid gap-4">
                 ...
                 <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
                     <label className="block text-slate-400 text-xs font-bold uppercase mb-2">Size (Big/Small) Multiplier</label>
                     <input type="number" value={settings.winRatioSize} disabled className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white" />
                 </div>
            </div>
            <p className="text-xs text-slate-500 italic">* Multipliers are read-only in this demo version.</p>
        </div>
    );
};